  ...
  <div :val="value_">
    ...
    <div style="margin-top: 5px">
      ...
      <el-checkbox-group v-model="appoint">
        <!-- eslint-disable -->
        <div  v-for="i in 4" :key="i" style="margin-left: 10px;  line-height: 16px;">
          <el-checkbox @change="type = '4'"  v-for="j in 10" v-if="parseInt((i - 1) + '' + (j - 1)) < 32 && !(i === 1 && j === 1)"
                       :key="j"
                       :label="(i - 1) + '' + (j - 1)"
                       style="margin-left: -8px"></el-checkbox>
        </div>
      </el-checkbox-group>
    </div>
  </div>
