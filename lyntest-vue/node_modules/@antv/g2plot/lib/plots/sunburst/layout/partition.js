"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var round_1 = require("./round");
var dice_1 = require("../../treemap/layout/dice");
var util_1 = require("@antv/util");
/** reference: https://github.com/d3/d3-hierarchy/blob/master/src/partition.js */
function patition(root) {
    calculateDepth(root);
    var dx = 1;
    var dy = 1;
    var isRound = false;
    // const n = root.height + 1;
    var n = 1;
    root.x0 = 0;
    root.y0 = 0;
    root.x1 = dx;
    root.y1 = dy / n;
    loopChildren(root, positionNode(dy, n));
    if (isRound) {
        loopChildren(root, round_1.round);
    }
    return root;
}
exports.default = patition;
function positionNode(dy, n) {
    return function (node) {
        if (node.children) {
            dice_1.dice(node, node.x0, (dy * (node.depth + 1)) / n, node.x1, (dy * (node.depth + 2)) / n);
        }
        var x0 = node.x0, y0 = node.y0, x1 = node.x1, y1 = node.y1;
        if (x1 < x0)
            x0 = x1 = (x0 + x1) / 2;
        if (y1 < y0)
            y0 = y1 = (y0 + y1) / 2;
        node.x0 = x0;
        node.y0 = y0;
        node.x1 = x1;
        node.y1 = y1;
    };
}
function loopChildren(root, callback) {
    var nodes = [root];
    var node = root;
    while ((node = nodes.pop())) {
        callback(node);
        var children = node.children;
        if (children) {
            for (var i = children.length - 1; i >= 0; --i) {
                nodes.push(children[i]);
            }
        }
    }
}
function calculateDepth(root) {
    root.x0 = 0;
    root.x1 = 1;
    var children = root.children;
    if (children) {
        util_1.each(children, function (n) {
            n.parent = root;
            n.depth = root.depth + 1;
            calculateDepth(n);
        });
    }
}
//# sourceMappingURL=partition.js.map